...
def createDictionaries():
    # dictionary creation
    IDsDict = {}  # this dictionary holds key-value pairs as commiterName-ID(s) *Helper Dictionary*
    nestedDict = {}  # the nested dictionary *Main dictionary*
    keysOFNames = {  # Values ,as a dictionary, of the main dictionary
        "SwM tasks": [0, 0, 0],
        "NFR Labeling": [0, 0, 0, 0, 0, 0],
        "SoftEvol tasks": [0, 0, 0, 0]
    }
    identifiesFileName = sys.argv[2]
    # identifiesFileName = "identifies.txt"
    try:
        file = open(identifiesFileName, "r")
        ...
        for item in splittedList:
            if item[1] not in nestedDict.keys():  # item[1] has name  of the committer
                nestedDict[item[1]] = copy.deepcopy(keysOFNames)

        # IDsDict
        for item in splittedList:
            if item[1] not in IDsDict.keys():  # item[1] has name  of the committer
                IDsDict[item[1]] = []
    ...
    commitsFileName = sys.argv[1]
    # commitsFileName = "commits.txt"
    try:
        file = open(commitsFileName, "r")
        ...
        for i in range(noOfCommits):
            CCF.append(splittedList[i][1:15])  # take data of splittedList, from index range 1:15
    ...
    return nestedDict,IDsDict

#First tasks of the menu
def compareParticularDeveloper(nestedDict, IDsDict):
    # lists available classifications
    nestedDict, IDsDict = createDictionaries()
    ...
    if (classificationScheme == "Swanson's Maintenance Tasks" or classificationScheme == "SwM"):
        ...
        features = ["Maintainability", "Usability", "Functionality", "Reliability", "Efficiency",
                    "Portability"]  # features of NFR Labeling
        ...
        features = ["Forward Engineering", "Re-Engineering", "Corrective Engineering",
                    "Management"]  # features of Software Evolution Tasks
...
#Second tasks of the menu
def compareAllDeveloper(nestedDict, IDsDict):
    nestedDict, IDsDict = createDictionaries()
    # lists available classifications
    print("\nAvailable classification schemes are as follows: ")
    print("Swanson's Maintenance Tasks (SwM)")
...
#Third tasks of the menu
def developerWithMaxCommits(nestedDict, IDsDict):
    nestedDict, IDsDict = createDictionaries()
    # lists available classifications
    print("\nAvailable classification schemes are as follows: ")
    print("Swanson's Maintenance Tasks (SwM)")
    ...
    print("Committer named " + maxValueCommitter + " has maximum #of commits with " + correspondingFeature + " with a value of " + str(
            maxCommitValue))

#Fourth tasks of the menu
def endMenu():
    print("Bye!")
    exit()

#Menu Function
def menu(nestedDict, IDsDict):
    nestedDict, IDsDict = createDictionaries()
    ...
    while (True):  # this loop continue until user enter 4 as an option.
        ...
        if (option == 1):
            compareParticularDeveloper(nestedDict, IDsDict)

        elif (option == 2):
            compareAllDeveloper(nestedDict, IDsDict)

        elif (option == 3):
            developerWithMaxCommits(nestedDict, IDsDict)
